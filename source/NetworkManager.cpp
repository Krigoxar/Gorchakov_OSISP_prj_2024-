...
void NetworkManager::initializePawns(uint32_t inPlayerId)
{
	Vector2 mSpawnPawnPos;
	if (ScoreBoardManager::sInstance->GetEntry(inPlayerId)->GetColor() == Color::WHITE)
	{
		mSpawnPawnPos = Vector2(6, 0);
	}
	else
	{
		mSpawnPawnPos = Vector2(1, 0);
	}

	for (int i = 0; i < 8; ++i)
	{
		mSpawnPawnPos.mY = i;
		SpawnPiece(inPlayerId, mSpawnPawnPos, PieceType::PAWN);
	}
}
...
void NetworkManager::initializePieces(uint32_t inPlayerId)
{
	...
	if (ScoreBoardManager::sInstance->GetEntry(inPlayerId)->GetColor() == Color::WHITE)
	{
		mSpawnPos = Vector2(7, 0);
	}
	else
	{
		mSpawnPos = Vector2(0, 0);
	}
	...
	for (int i = 0; i < 8; i++)
	{
		...
		case 'R':
			...
			break;
		case 'N':
			SpawnPiece(inPlayerId, mSpawnPos, PieceType::KNIGHT);
			break;
		case 'B':
			SpawnPiece(inPlayerId, mSpawnPos, PieceType::BISHOP);
			break;
		case 'Q':
			SpawnPiece(inPlayerId, mSpawnPos, PieceType::QUEEN);
			break;
		case 'K':
			SpawnPiece(inPlayerId, mSpawnPos, PieceType::KING);
			break;
		}
	}
}
...
void NetworkManager::SpawnPiece(uint32_t inPlayerId, const Vector2 &inSpawnVec, PieceType inPieceType)
{
	PiecePtr piece = std::static_pointer_cast<Piece>(GameObjectRegistry::sInstance->CreateGameObject('PIEC'));
	piece->SetColor(ScoreBoardManager::sInstance->GetEntry(inPlayerId)->GetColor());
	piece->SetPlayerId(inPlayerId);
	piece->SetLocation(Vector2(inSpawnVec.mX, inSpawnVec.mY));
...
NetworkManager::ReceivedPacket::ReceivedPacket(float inReceivedTime, InputMemoryBitStream &ioInputMemoryBitStream, const SocketAddress &inFromAddress) : mReceivedTime(inReceivedTime),
